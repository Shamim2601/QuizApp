...
describe('Unsuccessful Test Cases', () => {
  test('Question does not exist', () => {
    const res = request('POST', SERVER_URL + `/v2/admin/quiz/${quiz1.quizId}/question/${question.questionId + 53}/duplicate`, { headers: { token: user1.token } });
    expect(JSON.parse(res.body.toString())).toStrictEqual({ error: expect.any(String) });
    expect(res.statusCode).toStrictEqual(400);
  });

  test('Token is Invalid', () => {
    const res = request('POST', SERVER_URL + `/v2/admin/quiz/${quiz1.quizId}/question/${question.questionId}/duplicate`, { headers: { token: 'surelyinvalid' } });
    expect(JSON.parse(res.body.toString())).toStrictEqual({ error: expect.any(String) });
    expect(res.statusCode).toStrictEqual(401);
  });

  test('Quiz is Invalid', () => {
    const res = request('POST', SERVER_URL + `/v2/admin/quiz/${quiz1.quizId + 6231}/question/${question.questionId}/duplicate`, { headers: { token: user1.token } });
    expect(JSON.parse(res.body.toString())).toStrictEqual({ error: expect.any(String) });
...
describe('Successful Test Cases', () => {
  test('Valid Return Type + Side Effect', () => {
    const res = request('POST', SERVER_URL + `/v2/admin/quiz/${quiz1.quizId}/question/${question.questionId}/duplicate`, { headers: { token: user1.token } });
    console.log(JSON.parse(res.body.toString()));
    expect(JSON.parse(res.body.toString())).toStrictEqual({
      newQuestionId: expect.any(Number)
    });
    expect(res.statusCode).toStrictEqual(200);
    const res2 = request('GET', SERVER_URL + `/v2/admin/quiz/${quiz1.quizId}`, {
      headers: {
        token: user1.token,
      }
    });
