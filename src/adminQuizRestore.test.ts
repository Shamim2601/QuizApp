...
describe('Unsuccessful Cases', () => {
  test('Quiz not in trash', () => {
    ...
    quizId = JSON.parse(quizRes.body.toString()).quizId;
    const res = request('POST', SERVER_URL + `/v2/admin/quiz/${quizId}/restore`, { headers: { token: session } });
  ...
  test('Invalid Token', () => {
    quizId = null;
    const res = request('POST', SERVER_URL + `/v2/admin/quiz/${quizId}/restore`, { headers: { token: 'nowaythisisavalidtoken' } });
  ...
  test('Invalid Quiz Id', () => {
    ...
    const res = request('POST', SERVER_URL + `/v2/admin/quiz/${quizId + 900}/restore`, { headers: { token: session } });
...
describe('Successful Cases', () => {
  test('Successful Return Type + Test Back In Quizzes', () => {
    ...
    const res2 = request('GET', SERVER_URL + '/v2/admin/quiz/list', { headers: { token: session } });
