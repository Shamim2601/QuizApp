...
describe('Failed Logout', () => {
  test('Token Empty', () => {
    const res = request('POST', SERVER_URL + '/v1/admin/auth/logout', { json: { token: null } });
    expect(JSON.parse(res.body.toString())).toStrictEqual({ error: expect.any(String) });
    expect(res.statusCode).toStrictEqual(401);
  });

  test('Token does not exist', () => {
    const res = request('POST', SERVER_URL + '/v1/admin/auth/logout', { json: { token: "there'snowaythisisatokenright??????" } });
    expect(JSON.parse(res.body.toString())).toStrictEqual({ error: expect.any(String) });
    expect(res.statusCode).toStrictEqual(401);
  });
});
...
describe('Successful Logout + Side Effects', () => {
  test('Successful Return Type', () => {
    const res = request('POST', SERVER_URL + '/v1/admin/auth/logout', { json: { token: token } });
  ...
  test('Side Effect', () => {
    let res = request('POST', SERVER_URL + '/v1/admin/auth/logout', { json: { token: token } });
    expect(JSON.parse(res.body.toString())).toStrictEqual({});
    expect(res.statusCode).toStrictEqual(200);
