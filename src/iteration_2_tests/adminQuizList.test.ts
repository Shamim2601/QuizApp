...
describe(' Successful Cases', () => {
  test('No quizes owned by user', () => {
    const res = request('GET', SERVER_URL + '/v1/admin/quiz/list', { qs: { token: user1.token } });
  ...
  test('successfully lists quizes owned by one user', () => {
    ...
    const quiz3Res = request('POST', SERVER_URL + '/v1/admin/quiz', { json: { token: user1.token, name: 'third quiz', description: 'hard' } });
