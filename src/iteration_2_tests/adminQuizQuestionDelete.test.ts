...
describe('Failed question removal', () => {
  test('Not a valid token', () => {
    const res = request('DELETE', SERVER_URL + '/v1/admin/quiz/' + quiz.quizId + '/question/' + question.questionId, {
      json: {
        token: 'abcd',
      }
    });
  ...
  test('Not a valid quizId', () => {
    const res = request('DELETE', SERVER_URL + `/v1/admin/quiz/${quiz.quizId + 1}/question/${question.questionId}`, {
      json: {
        token: session,
      }
    });
  ...
  test('Not a valid questionId', () => {
    const res = request('DELETE', SERVER_URL + `/v1/admin/quiz/${quiz.quizId}/question/${question.questionId + 1}`, {
      json: {
        token: session,
      }
    });
